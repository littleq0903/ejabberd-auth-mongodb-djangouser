...
binstring(S) when is_list(S) ->
    fun_apply(S, fun list_to_binary/1, fun(X)->X end);
binstring(B) when is_binary(B) ->
    fun_apply(B, fun(S) -> S end, fun(X)->X end).


hexstring(S) when is_list(S) -> 
    fun_apply(S, fun list_to_binary/1, fun bin2hex/1);
hexstring(B) when is_binary(B) ->
    fun_apply(B, fun(S) -> S end, fun bin2hex/1).
